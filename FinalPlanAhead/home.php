  ...
  <div class="container-fluid">
    <div class="navbar-header">
      <button type="button" class="navbar-toggle" data-toggle="collapse" data-target="#myNavbar">
       ...
       <a class="navbar-brand" href="home.php"><div ><img src="logo.PNG" style="position:absolute;top:10px;height:75%;width:4%;"/>    </div>
    ...
    <ul class="nav navbar-nav" style="position:absolute;left:74px;">
       ...
       <ul class="nav navbar-nav navbar-right">
        <li><form method="post">
                    <button name="logout" style="position:absolute;top:7px;left:1760px;" class="btn btn-danger my-2">Logout</button>
            </form>
</li>
      </ul>
    </div>
...
<div style="position:absolute;left:60px;top:250px;">
	...
	{   
        $ceid=array();
        $sql1 = "SELECT events.eid FROM events INNER JOIN users on events.username=users.username where events.username= '$curr' UNION SELECT eid FROM organisers where collabOrg='$curr' "; // and users.type= '{$_SESSION['type']}' 
		...
		if($result-> num_rows > 0){
		  while($row = $result-> fetch_assoc()){
			  ...
			  echo '<div class="container">    
				...
				<a href="participantnext.php?var='.$v1.'"><button value="Join" style="margin-left:5px;margin-bottom:15px;color:white;background-color:gray;">JOIN</button></a>
				<div class="panel-footer"></div>
						</div>
					   </div>';}
    }
